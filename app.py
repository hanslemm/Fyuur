#I've looked into
#https://github.com/tiennynyle/Fyuur/blob/master/app.py
#To get inspiration on how to work around some areas that I was stuck
...
def venues():
  ...
  for venue in venues: 
    shows_available = Shows.query.filter_by(venue_id=venue.id)
    num_upcoming_shows = (shows_available.filter(Shows.start_time > dt.now())).count()
    venue_data= {
      'id': venue.id,
      'name': venue.name,
...
def search_venues():
  ...
  for venue in venues:
    ...
    numUpcomingShows = (showsAvail.filter(Shows.start_time > dt.now())).count()
    venue_data = {
      "id" : venue.id,
      'name' : venue.name,
...
def show_venue(venue_id):
  ...
  numShows = Shows.query.filter_by(venue_id=selectedVenue.id).all()
  listOfPastShows = []
  listOfUpcomingShows = []
...
def create_venue_submission():
  ...
  try:
    ...
    venue_created = Venue(name=name, city=city, state=state, address=address, phone=phone, image_link=image_link, facebook_link=facebook_link)
...
def show_artist(artist_id):
  ...
  numShows = Shows.query.filter_by(artist_id=selectedArtist.id).all()
  listOfPastShows = []
  listOfUpcomingShows = []
...
def create_artist_submission():
  ...
  try:
    ...
    facebook_link = request.form['facebook_link']
    image_link = request.form['image_link']
    artist_created = Artist(name=name, city=city, state=state, phone=phone, genres=genres, image_link = image_link, facebook_link=facebook_link)
...
def create_show_submission():
  ...
  try:
    ...
    show_created = Shows(artist_id=artist_id, venue_id=venue_id, start_time=start_time)
